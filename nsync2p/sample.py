...
class NSyncSample:
    def __init__(
        ...
        self.eventlog = eventlog
        self.extracted_signals = extracted_signals
        self.mat_no_frames = mat_no_frames
        self.animal_name = animal_name
        ...
        self.event_windows = self._align_event_windows()
    ...
    def _align_event_windows(self) -> NDArray[np.float64]:
        events = self._get_valid_events()
        ...
        num_trials = len(events)
        aligned_windows = np.full((num_trials, self.window_size, self.get_num_neurons()), np.nan, dtype=np.float64)
        signals_t = self.extracted_signals.T  # frames × neurons
        valid_trials = []
        for i, event_idx in enumerate(frame_indices):
            event_idx = int(event_idx)
            if event_idx < self.pre_window_size or event_idx >= (self.get_num_frames() - self.post_window_size):
                continue
            aligned_windows[i] = signals_t[event_idx - self.pre_window_size : event_idx + self.post_window_size]
            valid_trials.append(i)
        ...
        if valid_trials:
            ...
            aligned_windows = np.full((self.get_num_neurons(), self.window_size, 2), np.nan, dtype=np.float64)

        return aligned_windows

    def _get_frame_timestamps(self, mat_file: Union[str, Path] | None = None, animal: str = None) -> NDArray[np.float64]:
        frame_ts = self.event_timestamps[self.event_ids == 9]

        if frame_ts.size == 0:
            animal = animal or self.animal_name
            if animal in ['CTL1', 'ER-L1', 'ER-L2', 'IG-19', 'IG-28', 'PGa-T1', 'XYZ'] and mat_file is not None:
                try:
                    assumed_data = sio.loadmat(mat_file)
                    empty_eventlog = np.squeeze(assumed_data['eventlog'])
    ...
    def get_window_dimensions(self) -> tuple[int, int, int]:
        return self.pre_window_size, self.post_window_size, self.window_size
